...
export const OPENING_PLAN_SCHEMA_VERSION = 1 as const;
export const OPENING_PLAN_STORAGE_KEY = '人间修订中:开场配置方案库:v1';
const MAX_PLANS = 100;
...
export function readOpeningPlans(storage: Storage | null = getBrowserStorage()): OpeningPlan[] {
  if (!storage) return [];
  const raw = storage.getItem(OPENING_PLAN_STORAGE_KEY);
  if (!raw) return [];
  try {
    const payload = JSON.parse(raw);
    const migrated = migrateOpeningPlanLibraryPayload(payload);
    return openingPlanLibrarySchema.parse(migrated).plans;
  } catch (error) {
    console.warn('[人间修订中·世界配置] 本地方案库读取失败，将使用空方案库', error);
    return [];
  }
}
