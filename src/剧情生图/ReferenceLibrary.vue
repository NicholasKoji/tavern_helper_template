  ...
  await run(kind, async (signal, settings) => {
    ...
    const shot =
      kind === 'face'
        ...
        : `单人全身参考图，人物从头到脚完整清晰，脸部、腰线、四肢和鞋履无遮挡，身体比例真实连贯。这是同一人物在另一个时间、构图和光线下拍摄的独立新镜头；所附面部图只定义稳定身份、脸型与五官关系，具体神态由本次拍照方式重新设计。
...
[参考图约束]\n所附面部图是人物身份来源，不是待扩展的原始画布。拍照方式负责背景、光线、构图、神情、动作、仪态和表现力，穿搭方向负责人物造型，两者互不覆盖。保持单人、头脚完整、脸部清楚、身体无遮挡；背景和道具只作陪衬。避免僵硬立正、固定叉腰、统一抱臂、机械微笑，以及不同人物重复同一种模特姿势。这是参考图专用造型，不是角色永久服装，后续剧情穿搭、动作和表情仍以当楼正文为准。`;
        ...
        ? [
            {
              label:
                character.name +
                '已确认面部身份来源：只提取同一人物的稳定脸型、五官关系与辨识度；这不是待扩展的原始画布，后面的最高优先级协议将规定本次全身图的面部重绘。',
              url: character.face,
            },
          ]
        : [],
      { referencePromptMode: kind === 'body' ? 'full-body-reference' : 'scene' },
    );
  });
}
